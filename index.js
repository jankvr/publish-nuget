...
class Action {
    ...
    _pushPackage(version, name) {
        ...
        this._executeInProcess(`dotnet build -c Release ${this.projectFile} ${this.version ? "-p:Version=" + this.version : ""}`)

        this._executeInProcess(`dotnet pack ${this.includeSymbols ? "--include-symbols -p:SymbolPackageFormat=snupkg" : ""} --no-build -c Release ${this.projectFile} ${this.version ? "-p:Version=" + this.version : ""} -o .`)

        const packages = fs.readdirSync(".").filter(fn => fn.endsWith("nupkg"))
        console.log(`Generated Package(s): ${packages.join(", ")}`)

        const pushCmd = `dotnet nuget push *.nupkg -s ${this.nugetSource}/v3/index.json -k ${this.nugetKey} ${!this.includeSymbols ? "--no-symbols" : ""} --skip-duplicate ${this.projectFile}`,
